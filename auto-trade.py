...
# Enhanced Trade Log with P&L and P&L% and Sentiment Analysis
    if summary['trades']:
        ...
        if not trades_df.empty:
            ...
            # =======================================================================
            # [TỐI ƯU HÓA] BỎ TOÀN BỘ VÒNG LẶP O(N²) TÍNH P&L PHỨC TẠP TẠI ĐÂY
            # Vì summary['trades'] đã được xử lý chuẩn xác 100% qua hàm fix_trade_data().
            # =======================================================================
            
            """
            [COMMENT TOÀN BỘ VÒNG LẶP CŨ]
                        ...
                        if abs(profit) > 0.01:
                            ...
                            for buy_idx in reversed(range(idx)):
                                if buy_idx in buy_trades:
                                    ...
                                    break
            [KẾT THÚC COMMENT CODE CŨ]
            """

            # FORMAT HIỂN THỊ SIÊU NHANH (Vì Data đã sạch)
            ...
            def format_profit_pct(row):
                ...
                # Đảm bảo NaN hoặc các số siêu nhỏ (sai số float) hiển thị là 0.00%
                if pd.isna(profit_pct) or abs(profit_pct) < 0.001:
                    return "0.00%"
                ...
                if isinstance(val, str) and ('$' in val or '%' in val):
                    try:
                        ...
                        if numeric_val > 0.001:
                            return 'color: #00FF88; font-weight: bold'  # Green
                        elif numeric_val < -0.001:
                            return 'color: #FF4444; font-weight: bold'  # Red
                    except (ValueError, AttributeError):
                        pass
                return ''
            
            # Tự động render bảng giao diện
            try:
            ...
            # --- KẾT THÚC PHẦN BẢNG TRADE, TIẾP TỤC VỚI CODE CŨ BÊN DƯỚI (Sentiment Details) ---
...
def fix_trade_data(trades: list) -> list:
    """
    Standardize trade data, pair BUY-SELL orders, and calculate accurate P&L.
    
    Performance Optimization:
    - Reduced time complexity from O(N^2) to O(N) by using state tracking (last_buy_cost)
      instead of backward nested loops.
      
    Code Clarity:
    - Extracted Data Transformation logic out of the UI Presentation layer.
    - Used Immutability (trade.copy()) to prevent unintended side effects on the original data.
    """
    if not trades:
        return []

    fixed_trades = []
    # State variable to track the cost of the unmatched BUY order (O(1) lookup)
    last_buy_cost = 0.0  
    cumulative_profit = 0.0

    for trade in trades:
        # Create a shallow copy to maintain immutability of the original input
        cleaned_trade = trade.copy()
        action = cleaned_trade.get('action')

        if action == 'BUY':
            # Step 1: Store the cost of the BUY order to calculate percentage later
            cost = float(cleaned_trade.get('cost', 0.0))
            last_buy_cost = cost
            
            # BUY orders do not realize profit yet
            cleaned_trade['profit'] = 0.0
            cleaned_trade['profit_pct'] = 0.0
            
        elif action == 'SELL':
            # Step 2: Extract raw profit from the execution engine
            raw_profit = float(cleaned_trade.get('profit', 0.0))
            
            # Step 3: Handle Floating-Point Precision issues (e.g., $0.00000001 profit)
            if abs(raw_profit) < 0.01:
                profit = 0.0
                profit_pct = 0.0
            else:
                profit = raw_profit
                # Step 4: Calculate accurate Profit Percentage based on the paired BUY cost
                if last_buy_cost > 0:
                    profit_pct = (profit / last_buy_cost) * 100
                else:
                    profit_pct = 0.0

            # Update the dictionary with clean, presentation-ready data
            cleaned_trade['profit'] = profit
            cleaned_trade['profit_pct'] = profit_pct
            
            # Step 5: Update cumulative metrics and reset state for the next trade cycle
            cumulative_profit += profit
            last_buy_cost = 0.0  
            
        else:
            # Handle HOLD or unknown actions securely
            cleaned_trade['profit'] = 0.0
            cleaned_trade['profit_pct'] = 0.0

        # Attach cumulative profit for potential equity curve charting
        cleaned_trade['cumulative_profit'] = cumulative_profit
        fixed_trades.append(cleaned_trade)
...
def run_trading_simulation(symbol_input, interval, start_date, end_date, initial_capital, 
    ...
    try:
        ...
        # SỬ DỤNG HÀM VỪA TẠO ĐỂ CHUẨN HÓA DATA TRƯỚC KHI VẼ UI
        summary['trades'] = fix_trade_data(summary['trades']) 
